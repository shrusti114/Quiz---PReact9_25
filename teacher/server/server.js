...
let db, teacherCollection, subjectCollection, departmentCollection, quizCollection, questionCollection;
...
app.post("/login", async (req, res) => {
  try {
    ...
    const teacher = await teacherCollection.findOne({ teacherEmail: email, teacherPassword: password });
    if (!teacher)
...
app.post("/teachers/fetchByEmail", async (req, res) => {
  try {
    ...
    if (!teacher) return res.status(200).json({ success: false, message: "Teacher not found" });

    // Fetch subjects from teacher's department
    const subjects = await subjectCollection.find({ department_id: teacher.department_id }).toArray();
...
app.get("/departments/:department_id", async (req, res) => {
  try {
    ...
    if (!department) return res.json({ success: false, message: "Department not found" });
