...
const TeacherRegister = () => {
  ...
  const fetchDepartments = async () => {
    try {
      const res = await axios.get("http://localhost:5002/departments/display");
  ...
  const fetchTeachers = async () => {
    try {
      const res = await axios.get("http://localhost:5002/teachers/display");
  ...
  const handleAddOrUpdate = async () => {
    try {
      ...
      if (editId) {
        await axios.put("http://localhost:5002/teachers/update", {
          teacher_id: editId,
          teacherName,
          teacherEmail,
          teacherPassword,
        ...
        await axios.post("http://localhost:5002/teachers/insert", {
          teacherName,
          teacherEmail,
          teacherPassword,
          department_id: selectedDept,
  ...
  const handleDelete = async (id) => {
    await axios.delete(`http://localhost:5002/teachers/delete?id=${id}`);
